    ...
    public void configurate(LoggerContext loggerContext, String key, String value) {
        if (endsWithIgnoreCase(key, APPENDERS_SUFFIX)) {
            ...
            effector(loggerContext, key, ROLLING_FILE_LEVEL_SUFFIX)
                    .setEffectorLevel(RollingFileEffectorBuilder.ROLLING_FILE_EFFECTOR, Level.toLevel(value));
            ...
            addAppenderIfAbsent(rollingFileAppender(logger(loggerContext,
                    key, ROLLING_FILE_PRUDENT_SUFFIX)).setPrudent(toBoolean(value)));

        } else if (endsWithIgnoreCase(key, ROLLING_FILE_APPEND_SUFFIX)) {
            addAppenderIfAbsent(rollingFileAppender(logger(loggerContext,
                    key, ROLLING_FILE_APPEND_SUFFIX)).setAppend(toBoolean(value)));
            ...
            addAppenderIfAbsent(rollingFileAppender(logger(loggerContext,
                    key, ROLLING_FILE_IMMEDIATE_FLUSH_SUFFIX)).setImmediateFlush(toBoolean(value)));
            ...
            addAppenderIfAbsent(rollingFileAppender(logger(loggerContext,
                    key, ROLLING_FILE_CLEAN_HISTORY_ON_START_SUFFIX)).setCleanHistoryOnStart(toBoolean(value)));
        }
    }
}
